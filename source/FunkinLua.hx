	...
	public function new(script:String) {
		...
		try{
			...
			if(resultStr != null && result != 0) {
				trace('An Error Occurred When Loading A lua script!' + resultStr);
				#if windows
				lime.app.Application.current.window.alert(resultStr, 'An Error Occurred When Loading A lua script!');
				#else
				luaTrace('An Error Occurred When Loading A lua script!: "$script"\n' + resultStr, true, false, FlxColor.RED);
				#end
				lua = null;
				return;
			}
